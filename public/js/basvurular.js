...
(function () {
  ...
  async function openDetail(id) {
    ...
    contentEl.innerHTML =
      '<div class="alert alert-info">Detay yükleniyor...</div>';
    ...
    try {
      ...
      if (!res.ok) {
        contentEl.innerHTML = `<div class="alert alert-error">${a.error || 'Başvuru yüklenemedi'}</div>`;
        return;
      }
      ...
      contentEl.innerHTML = `
        ...
        <div class="detail-card">
          ...
          <div style="margin-top:18px;display:flex;gap:8px;flex-wrap:wrap;">
            ...
            <button class="btn btn-ghost" id="btn-kabul" data-status="kabul">Kabul Et</button>
            <button class="btn btn-ghost" id="btn-red" data-status="red">Reddet</button>
            <button class="btn btn-ghost" id="btn-tamam" data-status="tamamlandi">Tamamlandı</button>
          </div>
      ...
      contentEl.querySelectorAll('button[data-status]').forEach((btn) => {
        btn.addEventListener('click', async () => {
          ...
          if (note === null) return;
          const r = await fetch(`/api/applications/${a.id}/decision`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ status: btn.dataset.status, decisionNote: note }),
          });
          if (r.ok) {
            await loadList();
            openDetail(a.id);
      ...
      contentEl.innerHTML =
        '<div class="alert alert-error">Detay yüklenirken hata oluştu.</div>';
    }
  }
  ...
  async function loadList() {
    ...
    try {
      ...
      listEl.innerHTML =
        '<div class="alert alert-error">Başvurular yüklenirken hata oluştu.</div>';
    }
  }
